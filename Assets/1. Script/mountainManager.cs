...
public class mountainManager : MonoBehaviour 
{
	...
	public GameObject mountainHeightLimit;
	...
	void Start() 
	{
		...
		// 현진 추가
		origin = mountain.transform.position;
		trans = mountainHeightLimit.transform.position;
	}
	...
	public void purchasedItem() 
	{
		...
		if (isShow)
		{
			...
			if (level <= 50)
				mountain.transform.position = Vector3.Lerp(origin, trans, level * 0.02f);
			else if (level <= 450)
				mountain.GetComponent<MeshRenderer>().material.color = Color.Lerp(canvas.level0.color, canvas.level2.color, (level - 50) * 0.0025f);
		}
	} 
